...
@st.cache_data
def init_data():
    names_df = pd.read_csv("Dataset/samp_names2.csv")
    names_namecolname = 'name'
    return names_df,names_namecolname
...
def concat_name(selected_df):
    ...
    for t_list in stqdm(text_list):
        z = ['' if pd.isna(x) else x for x in t_list]
        text = ''.join(z)
        concated_names.append(text)
    return concated_names
# st.session_state.app1_dataframe,st.session_state.app1_name_column = init_data()
# st.session_state.app1_regex = True
def click_evaluation():
    st.session_state.app1_MockupEvaluation = True
...
if st.session_state.app1_upload == False:
    ...
    with first_section.container():
        ...
        if uploaded_file is not None:
            ...
            if optional_concat:
                option = st.multiselect(label = '',options = dataframe.columns.values,default = None)
            elif not optional_concat:
                option = st.selectbox('Which is Names Column ?',box_list)
            if option is not None:
                if isinstance(option,list):
                    dataframe['concated_name'] = concat_name(dataframe.filter(option))
                    #st.write(dataframe)
                    st.session_state.app1_name_column = 'concated_name'
                else:
                    st.session_state.app1_name_column = option
                start_name_classify =  st.button('Submit To Start')
                if start_name_classify:
                    st.session_state.app1_dataframe,st.session_state.app1_name_column = init_data_upload(dataframe,st.session_state.app1_name_column)
                    st.session_state.app1_regex = True
                    first_section.empty()
